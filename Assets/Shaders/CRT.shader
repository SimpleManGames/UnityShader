...
#define PI 3.141592653589
	uniform sampler2D _MainTex;
            ...
            // 0.1f
	uniform float _Gamma = 1.0f;
            // 1.0f
	uniform float _curvatureSet1 = 0.5f;
            // 0.5f
	uniform float _curvatureSet2 = 0.5f;
            // 0.5f
	uniform float _YExtra = 0.5f;
            // 0.5f;
	uniform float _rgb1R = 1.0f;
            // 1.0f
	uniform float _rgb1G = 1.0f;
            // 1.0f
	uniform float _rgb1B = 1.0f;
            // 1.0f
	uniform float _rgb2R = 1.0f;
            // 1.0f
	uniform float _rgb2G = 1.0f;
            // 1.0f
	uniform float _rgb2B = 1.0f;
            // 1.0f
	uniform float _dotWeight = 2.0f;
            // 2.0f

	// This bends the screen
	float2 RadialDistortion(float2 coord)
	{
                // Gets the coord we are changing
		coord *= _TextureSize / _InputSize;
                // Get the new coord based off the original location and how much the curve bends it
		float2 cc = coord - _curvatureSet1;
                float dist = dot(cc, cc) * _Distortion;
                // Returns the new pixel location based on the camera to world view
